...
from app.models import Match, MatchCreate, MatchCreateIn, MatchUpdate, OrderCreate
...
def update_tickets_left(session: Session, order_create: OrderCreate):
    match = session.exec(select(Match).where(Match.id == order_create.match_id)).first()
    match.total_available_tickets -= order_create.tickets_bought
    session.add(match)
    session.commit()
