...
@router.put("/{team_name}", dependencies=[Depends(get_current_active_superuser)])
async def update_team(team_data: TeamCreate|TeamUpdate, team_name: str, session: SessionDep):
    team = crud.team.get_team_by_name(session=session, name=team_name)
